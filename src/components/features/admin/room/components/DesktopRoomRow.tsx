...
export function DesktopRoomRow({ room }: { room: Room }) {
  ...
  const iconBg = {
    ...
    perbaikan: "bg-surface-container-highest text-on-surface-variant",
  }[room.status];
  ...
  return (
    <div className="flex flex-col gap-4 border-t border-border/30 px-6 py-4 transition-colors hover:bg-muted/30 md:flex-row md:items-center md:gap-0">
      ...
      <div className="flex-1 text-sm text-on-surface">Lantai {room.floor}</div>

      <div className="flex-1 text-sm text-on-surface">{formatCurrency(room.price)}</div>
